...
export default function App() {
    ...
    <div style={{minHeight:"100vh",background:C.bark,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:C.cream,borderRadius:24,padding:"40px 28px",maxWidth:360,width:"100%",textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,0.35)"}}>
        ...
        <input
          ...
          autoFocus
          style={{width:"100%",padding:"13px 16px",borderRadius:12,border:`2px solid ${C.light}`,fontSize:"1rem",outline:"none",marginBottom:13,background:C.sand,color:C.ink,boxSizing:"border-box",fontFamily:"inherit"}}
        />
        <button onClick={handleSetName} style={{width:"100%",padding:13,borderRadius:12,background:C.bark,color:C.sand,border:"none",fontSize:"1rem",fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
          Entrar al viaje →
        </button>
      </div>
    </div>
  ...
  if (!data) return (
    <div style={{minHeight:"100vh",background:C.cream,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      ...
      <div style={{color:C.clay,fontFamily:"sans-serif"}}>Cargando...</div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:C.cream,color:C.ink,fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:"22px 13px 80px"}}>
          ...
          <div>
            <div style={{background:"white",borderRadius:16,border:`1.5px solid ${C.light}`,padding:16,marginBottom:13,boxShadow:"0 2px 12px rgba(90,60,30,0.06)"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                {members.length===0
                  ? <div style={{fontSize:"0.82rem",color:"#9A8060",fontStyle:"italic"}}>Nadie se ha unido aún.</div>
                  : members.map((m,i)=>(
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:9,marginBottom:13}}>
              {[["Total",totalExp.toFixed(2)+"€"],["Por persona",(totalExp/(members.length||1)).toFixed(2)+"€"],["Gastos",expenses.length]].map(([label,val])=>(
            ...
            <div style={{background:"white",borderRadius:16,border:`1.5px solid ${C.light}`,padding:16,marginBottom:13,boxShadow:"0 2px 12px rgba(90,60,30,0.06)"}}>
              ...
              {expenses.length===0 && <div style={{textAlign:"center",color:"#9A8060",fontSize:"0.84rem",padding:18}}>Sin gastos todavía</div>}
              {expenses.map((exp,i)=>{
                const cat = CAT[exp.cat]||CAT.otro;
                const sw = exp.splitWith?.length||members.length;
              ...
              <AddBtn style={{marginTop:13}} onClick={()=>{
                ...
                update(d=>{if(!d.expenses)d.expenses=[];d.expenses.push({id:uid(),desc:"",cat:"otro",amount:0,paidBy:userName,splitWith:[...members]});return d;});
                setEditExp(idx);
              }}>＋ Añadir gasto</AddBtn>
            </div>
        ...
        {tab==="checklist" && (
          <div>
            {[{title:"🎒 ¿Qué llevamos?",items:checklist,key:"checklist"},{title:"📋 Antes de salir",items:preList,key:"preList"}].map(({title,items,key})=>(
      ...
      {/* MODAL EDITAR GASTO */}
      {editingExpense!==null && expenses[editingExpense] && (()=>{
        const exp = expenses[editingExpense];
            ...
            <div className="modal-box" onClick={e=>e.stopPropagation()}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:13}}>
                ...
                <div>
                  ...
                  <select value={exp.cat} onChange={e=>upd(ex=>{ex.cat=e.target.value;})}
                    style={{width:"100%",padding:"11px 14px",borderRadius:12,border:`1.5px solid ${C.light}`,fontSize:"0.87rem",background:C.sand,cursor:"pointer",fontFamily:"inherit"}}>
              ...
              <div style={{marginBottom:20}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:7}}>
                    <button onClick={()=>upd(ex=>{ex.splitWith=[...members];})} style={{fontSize:"0.7rem",color:C.sky,background:"transparent",border:`1px solid ${C.sky}`,borderRadius:100,padding:"2px 8px",cursor:"pointer",fontFamily:"inherit"}}>Todos</button>
                    <button onClick={()=>upd(ex=>{ex.splitWith=[];})} style={{fontSize:"0.7rem",color:C.sunset,background:"transparent",border:`1px solid ${C.sunset}`,borderRadius:100,padding:"2px 8px",cursor:"pointer",fontFamily:"inherit"}}>Ninguno</button>
                  </div>
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                  {members.map((m,mi)=>{
              ...
              <div style={{display:"flex",gap:9}}>
                <button onClick={()=>{showToast("✓ Guardado");setEditExp(null);}}
                  style={{flex:1,padding:"13px",borderRadius:12,background:C.bark,color:C.sand,border:"none",fontSize:"0.9rem",fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
                  Guardar
                </button>
                <button onClick={()=>{update(d=>{d.expenses.splice(i,1);return d;});setEditExp(null);showToast("🗑 Eliminado");}}
                  style={{padding:"13px 16px",borderRadius:12,background:"#FFF0F0",color:C.sunset,border:`1.5px solid #F0C0C0`,fontSize:"0.88rem",cursor:"pointer",fontFamily:"inherit"}}>
                  🗑
                </button>
              </div>
            </div>
